...
#include "tcurrentRecord.h"

WX_DECLARE_OBJARRAY( tcurrentRecord *, ArrayOftcurrentRecordPtrs );
...
class tcurrentRecordSet {
public:
    tcurrentRecordSet() {
        for(int i=0; i<Idx_COUNT; i++)
            m_tcurrentRecordPtrArray[i] = NULL;
    }

    time_t m_Reference_Time;
    tcurrentRecord *m_tcurrentRecordPtrArray[Idx_COUNT];
};
