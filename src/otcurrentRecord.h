/**********************************************************************
zytcurrent: meteorological tcurrent file viewer
Copyright (C) 2008 - Jacques Zaninetti - http://www.zytcurrent.org
...
/******************************************
Elément de base d'un fichier tcurrent
******************************************/

#ifndef tcurrentRECORD_H
#define tcurrentRECORD_H
...
#define tcurrent_NOTDEF -999999999
...
{
    public:
        tcurrentRecord(ZUFILE* file, int id_);
        tcurrentRecord(const tcurrentRecord &rec);
        tcurrentRecord(const tcurrentRecord &rec1, const tcurrentRecord &rec2, double d);
        static tcurrentRecord *MagnitudeRecord(const tcurrentRecord &rec1, const tcurrentRecord &rec2);
        ~tcurrentRecord();
        ...
        double getValue(int i, int j) const  { return ok ? data[j*Ni+i] : tcurrent_NOTDEF;}
        ...
        inline double  getX(int i) const   { return ok ? Lo1+i*Di : tcurrent_NOTDEF;}
        inline double  getY(int j) const   { return ok ? La1+j*Dj : tcurrent_NOTDEF;}
    ...
    private:
        ...
        // Data Access
        //---------------------------------------------
        bool readtcurrentSection0_IS (ZUFILE* file, bool b_skip_initial_tcurrent);
        bool readtcurrentSection1_PDS(ZUFILE* file);
        bool readtcurrentSection2_GDS(ZUFILE* file);
        bool readtcurrentSection3_BMS(ZUFILE* file);
        bool readtcurrentSection4_BDS(ZUFILE* file);
        bool readtcurrentSection5_ES (ZUFILE* file);
...
//==========================================================================
inline bool   tcurrentRecord::hasValue(int i, int j) const
{
...
//-----------------------------------------------------------------
inline bool tcurrentRecord::isPointInMap(double x, double y) const
{
...
//-----------------------------------------------------------------
inline bool tcurrentRecord::isXInMap(double x) const
{
//    return x>=Lo1 && x<=Lo1+(Ni-1)*Di;
//printf ("%f %f %f\n", Lo1, Lo2, x);
    if (Di > 0) {
        double maxLo = Lo2;
        if(Lo2+Di >= 360) /* tcurrent that covers the whole world */
            maxLo += Di;
...
//-----------------------------------------------------------------
inline bool tcurrentRecord::isYInMap(double y) const
{
    if (Dj < 0)
        return y<=La1 && y>=La2;
    else
